...
Orc.AutomaticSupport is a WildGums library that allows users to easily download a remote file that will validate and provide automatic support to the end-user. It provides an `IAutomaticSupportService` that downloads a support application from a URL and executes it, reporting progress along the way.
...
|------------|-------------------|
| Orc.AutomaticSupport | `master` |
| Orc.AutomaticSupport | `develop` |
...
Orc.AutomaticSupport is a WPF library targeting `net8.0-windows`, `net9.0-windows`, and `net10.0-windows`.

```
Orc.AutomaticSupport      => Main library (services, views, view models)
Orc.AutomaticSupport.Tests => Unit and integration tests
```
...
| `src/Orc.AutomaticSupport/Services/` | Yes | Core service implementations |
| `src/Orc.AutomaticSupport/Services/Interfaces/` | Yes | Service interfaces |
| `src/Orc.AutomaticSupport/ViewModels/` | Yes | MVVM view models |
| `src/Orc.AutomaticSupport/Views/` | Yes | WPF views |
